...
from components.modify_widget.metadata_setting import MetadataSetting
from ui_source.MetadataWidget import Ui_MetadataWidget
from components.modify_widget.modify_widget import ModifyWidget
...
class MetadataWidget(QWidget, Ui_MetadataWidget):
    ...
    def result_click_event(self, item) -> None:
        ...
        info = sm.get_song_metadata(path)
        # print(sm.compare_song_info(info, self.song_info))
    ...
    def write_event(self, *args, pic_buffer: io.BytesIO = None) -> None:
        ...
        if self.song_info:
            ...
            # todo 特殊的颜色标注
            new_md5 = sm.get_md5(file_path)
            if self.is_replace_old_md5:
                if self.is_download_lrc and os.path.exists(LRC_PATH + old_md5 + ".mrc"):
                    os.remove(LRC_PATH + old_md5 + ".mrc")
                sm.update_md5_to_file(old_md5, new_md5)

            if self.is_download_lrc:
                time.sleep(0.1)
                song_id_or_md5 = self.search_tableWidget.item(self.search_tableWidget.currentItem().row(), 3).text()
                self.download_lrc(song_id_or_md5, new_md5)
